...
void loop(){
 // pinMode(0,OUTPUT);
  // read tilt sensor input
  tiltOn=digitalRead(2);
  if (!tiltOn){
    if (PreviouslyOn){
      unsigned long currnetTime = millis();
        interval=currnetTime -lastTime;
      lastTime=millis();
      speedcnt=map(interval,10,2000,1,10);
      speedcnt=constrain(speedcnt, 0,10);
...
void blinkLoop(){
  currentLED++;
  currentLED=currentLED%12;
    for (int x=1;x<254;x++) spwm(x,currentLED,speedcnt);
    for (int x=254;x>1;x--) spwm(x,currentLED,speedcnt);
...
void slowLoop(){
  for (int i=1;i<13;i++) {
    for (int x=1;x<254;x++) spwm(x,i,5);
    for (int x=254;x>1;x--) spwm(x,i,5);
  }
}
